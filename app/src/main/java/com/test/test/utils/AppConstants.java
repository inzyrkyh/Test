package com.test.test.utils;

/**
 * Created by bai on 2015/11/4.
 */
public class AppConstants {
    public final static int iRetError = 0xEFFFFFFF;
    public final static String sRetError = "son of bitch";

    public static String TestKey = "TestKey";
    public static String AppUrl1 = "http://42.96.138.5:8080/cgi-bin/hello.cgi";
    public static String AppUrl2 = "http://42.96.138.5:8080/cgi-bin/hello.cgi";
    //public static String AppUrl2 = "http://192.168.2.200:9785/MyHellTest//hi";
    public static String NetGetKey = "NetGetKey";
}
